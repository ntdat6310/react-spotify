...
import { Albums as AlbumsType } from 'src/types/album.type'
      ...
      {albums?.items.map((album) => (
        ...
        >
          ...
          <div className='flex h-full grow flex-col justify-around'>
            <span className='line-clamp-1 capitalize text-white'>{album.album.name}</span>
            <div className='flex items-center'>
              <span className='capitalize'>{album.album.type}</span>
              <HiUser className='ml-3 mr-1 h-4 w-4' />
              <span className='line-clamp-1'>
                {album.album.artists && album.album.artists.length > 0 && album.album.artists[0].name}
              </span>
            </div>
          </div>
        </NavLink>
